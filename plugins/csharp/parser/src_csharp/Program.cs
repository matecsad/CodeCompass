...
namespace CSharpParser
{
  class Program
  {
    ...
    private static string _dbSystem = "";
        ...
        List<SyntaxTree> trees, CSharpCompilation compilation)
    {
      ...
      CsharpDbContext dbContext = new CsharpDbContext(_dbSystem, csharpConnectionString);
      var contextList = new List<CsharpDbContext>();
      contextList.Add(dbContext);
      for (int i = 1; i < threadNum; i++)
      {
        CsharpDbContext dbContextInstance = new CsharpDbContext(_dbSystem, csharpConnectionString);
